...
from utils.email_processor import process_email, get_abvrs
...
async def get_abvrs_from_keywords_endpoint(refresh_keywords_request: RefreshKeywordsRequest) -> Dict[str, Any]:
    keywords_array, selected_audiences, left_audiences = get_abvrs("", "", refresh_keywords_request.cohorts, refresh_keywords_request.keywords)
    return {
...
async def get_forecast_endpoint(forecast_request: ForecastRequest) -> Dict[str, Any]:
    try:
        ...
        cohorts = get_cohorts()
        cohort_list = forecast_request.cohorts
        abvr_set = []
        seen = set()
        for cohort in cohort_list:
            if cohort not in cohorts:
                return {"error": f"Invalid cohort: {cohort}."}
            for abvr in cohorts[cohort]['abvrs'].split(","):
                abvr = abvr.strip()
        ...
        result = get_forecast_data(abvrs, forecast_request.locations, forecast_request.preset, forecast_request.creative_size, forecast_request.device_category, forecast_request.duration)
